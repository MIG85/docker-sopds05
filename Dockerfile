FROM python:3.11.11-alpine3.19 AS build-stage
...
FROM python:3.11.11-alpine3.19 AS production-stage
LABEL maintainer="mail@zveronline.ru"
...
COPY scripts/sopds.conf /etc/apache2/conf.d/sopds.conf

RUN apk add --no-cache -U bash libxml2 libxslt libffi libjpeg zlib postgresql expect apache2 apache2-mod-wsgi\
    && chmod +x /start.sh
